...
###################### TAF Global Variables ###############################
	...
	my $tags		="_full_,_smoke_,_smoketest_,_regression_,_regressiontest_,_bat_,_32bit_,_64bit_,_list_,_load_,_erase_";
	my $TSHookIsPerl	= "n";
	my $http_port		= ":8080";
...
sub printTAFGlobalVars { open Fout, ">$c/$_TAF/_tafGlobalVars.txt"; print Fout &printGlobalVars(); close Fout; print " -> $c/$_TAF/_tafGlobalVars.txt\n"; }
sub printTAFTSVars { open Fout, ">$c/$_TAF/$SvrProjName/_tafGlobalVars.txt"; print Fout &printTSGlobalVars(); close Fout; print " -> $c/$_TAF/$SvrProjName/_tafGlobalVars.txt\n"; }
sub printTAFTCVars { open Fout, ">$c/$_TAF/$SvrProjName/$SvrTCNamePattern/_tafGlobalVars.txt"; print Fout &printTCGlobalVars(); close Fout; print " -> $c/$_TAF/$SvrProjName/$SvrTCNamePattern/_tafGlobalVars.txt\n"; }
...
sub updateTAFTSVars     { my $str="Execution_24_7_title          =n\nExecution_local_only           =y\n";  $str = shift if @_; 
			...
			if (-e "$c/$_TAF/$SvrProjName/_tafGlobalVars.txt") {
			open Fout, ">>$c/$_TAF/$SvrProjName/_tafGlobalVars.txt"; print Fout $str; close Fout; print " ->> $c/$_TAF/$SvrProjName/_tafGlobalVars.txt\n"; 
			} else { print "updateTAFTSVars: $c:/$_TAF/$SvrProjName/_tafGlobalVars.txt does *NOT* exist\n"; }
			}

sub updateTAFTCVars     { my $str="Execution_24_7_title          =n\nExecution_local_only           =y\n";  $str = shift if @_; 
			if (-e "$c/$_TAF/$SvrProjName/$SvrTCNamePattern/_tafGlobalVars.txt") {
			open Fout, ">>$c/$_TAF/$SvrProjName/$SvrTCNamePattern/_tafGlobalVars.txt"; print Fout $str; close Fout; print " ->> $c/$_TAF/$SvrProjName/$SvrTCNamePattern/_tafGlobalVars.txt\n"; 
			} else { print "updateTAFTCVars: $c:/$_TAF/$SvrProjName/$SvrTCNamePattern/_tafGlobalVars.txt does *NOT* exist\n"; }
			}
...
<META http-equiv="refresh" content="$htmlRefreshRate"> 
		<HTA:APPLICATION ID="oMyApp" 
		    ...
		    SCROLL="no"
		    WINDOWSTATE="normal">
<body><pre> <h2><a href="$urlHttp/$reportHtml_http" title="Click to view *all* Testsuites"> Test Automation </a>$indexTitleRefreshCGI ($hostname/$ip)</h2>
EOF
...
#    ############################## Read Property File to update reportHtmlHistory ###########################
	...
	$stopTS    = "<a href=\"${urlHttp}/$tafCGI?tafArgs=Execution_from_cgi=y;Execution_local_only=$Execution_local_only;exitTAFGracefullyString=$tcname;exitTAF=_null_;tafRedirectory=${urlHttp}/$SvrProjName/$reportHtml_http\" title=\"Stop Test (Testcase Number = $tcSerialN)\" >.</a>";
...
sub generateExcelReport {
	...
	#  Todo: todo: need some variable initialization 
	...
	#&processTCs ("","testsuite=$SvrProjName;AutomationtsName=$SvrProjName;resetTSFileName=_list_tcDesc.txt;resetTSFile;propertyExecCtrEqZero=y;propertyOp=list_tcDesc;propertyOp=_null_"); 
	#&processTCs ("","testsuite=$SvrProjName;AutomationtsName=$SvrProjName;resetTSFileName=_list_history_tcRunResult.txt;resetTSFile;propertyExecCtrEqZero=y;propertyOp=list_history_tcRunResult;propertyOp=_null_");

	# bug: _tc_Desc.txt and _list_history_tcRunResult are not genreated 
	#   my $rst=  &processTCs ("","testsuite=$SvrProjName;AutomationtsName=$SvrProjName;resetTSFileName=_list_tcDesc.txt;resetTSFile;NofPropertyExecCtr=0;propertyOp=list_tcDesc;"); 
	#      $rst   =	&processTCs ("","testsuite=$SvrProjName;AutomationtsName=$SvrProjName;resetTSFileName=_list_history_tcRunResult.txt;resetTSFile;NofPropertyExec=0;propertyOp=list_history_tcRunResult;");
...
REM $c\\$_TAF\\taf.pl testsuite=_testsuite1_;printTAFTSVars
REM $c\\$_TAF\\taf.pl testsuite=_testsuite1_;testcase=testcase0003;printTAFTCVars

REM _TAF\\_testsuite\\TAFTSVars.txt that overwrite _TAF\\TAFGlobalVARS.txt
$c\\$_TAF\\taf.pl testsuite=_testsuite1_;printTAFTSVars
$c\\$_TAF\\taf.pl testsuite=_testsuite1_;updateTAFTSVars
$c\\$_TAF\\taf.pl testsuite=_testsuite2_;testsuite=testsuite0002;updateTAFTCVars
...
rem $c\\$_TAF\\taf.pl testsuite=_testsuite2_;testcase=testcase0002;clearTAFTCVars
$c\\$_TAF\\taf.pl testsuite=_testsuite2_;testcase=testcase0002;printTAFTCVars
$c\\$_TAF\\taf.pl testsuite=_testsuite2_;testcase=testcase0002;updateTAFTCVars
$c\\$_TAF\\taf.pl testsuite=_testsuite2_;list
...
REM Generate index.ps1 -> index.pl for the original TC
...
\$TAF->processTCs(\$cmdLine) if \$cmdLine;
EOF
...
my \$q = new CGI;
my \$tafArgs = "help";
   \$tafArgs = \$q->param("tafArgs");  \$tafArgs =~ s/_semi_/;/g;
...
if ((\$tafArgs =~ /Execution_local_only=y/i) && (\$tafArgs =~ /Execution_from_cgi=y/i)) {
	print "This test can NOT be executed over the network. Pleae execute the test from $hostname/$ip. \\n" ;
} else {
...
<pre>
<p>
<h2> ${titleStatus}${indexTitle}($hostname/$ip)$listTSUnderTest</h2>
${indexResult}${listTAFTestBed}${updateTS}${listTSAll}${indexthProperty}${tafGlobalVars}${indexCmdLog} ${stopTS}${markTS}${execTS} <a title=\"Click Testcase Desc to view TC logs\">$tcDescTitle      ${indexPassed}${indexOthers}${indexFailed}       ${indexSeconds}       ${indexUrlIIS}  </span>
EOF
...
# * Improvement: Testsuite search won't take regexp (as testcase does) 
...
# * Bug see Todo: &processTCs won't gernerate _list_tcDesc.txt 
