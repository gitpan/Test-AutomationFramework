...
###################### TAF WebUI Default Settings ###############################
	...
	if    (`ping $hostname` =~ /Ping request could not/i) { $web_ui_title_tip = "Click to update every TAF testsuites";}
	elsif (`ping $hostname` =~ /unknown/i)                { $web_ui_title_tip = "Click to update every TAF testsuites";}
	else {
...
sub recursiveSearchtcMain_1() { 
	...
	{ 	
		...
		if ($property[$#property -1] =~ /$tags_/ ) {;}
		else {		# stop recursivly creating property test suite
...
$SvrProjName=$SvrProjName_;
$SvrProjName =~ s/\\/\//g;
&readTAFProperty();
&generateRootIndex("exit");
1;
}
...
sub recursiveSearchtcMain_2() { 
	...
	{ 	
		...
		my $cmd = $File::Find::name; # , $reportHtml\n";
		$cmd =~ s/\/?$reportHtml//;
		$cmd =~ s/$c\/$_TAF//;
		$cmd =~ s/^\s*\///;
		if ($cmd) { $cmd = "testsuite=$cmd;list"; 
...
find(sub { if ($File::Find::name =~ /index\.htm$/i) {
 	...
 	foreach my $each1 (sort keys %tafUI) { print INDEX $tafUI{$each1} if ($each1 !~ /^\s*$c\/$_TAF/); }
...
sub reportTCSummary {
	...
	$reportHtmlSummaryStr = "<html><body><pre>\n"; $reportHtmlSummaryStr = $reportHtmlSummaryStr . "<META http-equiv=\"refresh\" content=\"$htmlRefreshRate\"> ";
...
EOF
	...
	if ( &enterY("Execute $c/taf.bat (y/n)? ") =~ /y/) {	system $cmd; }
	}
	1;
}
...
sub prHtml1_strGen_CGI() {
	...
	#my $perl = $^X;  $perl =~ s/\\/\\\\/g; # todo 11
	# my $perl = $^X;  $perl =~ s/\\/\\\//g; # todo 11
	my $execTS =  sprintf("<a href=\"${localUrl}/$tafCGI?tafArgs=SysDrive=$SvrDrive;Execution_24_7=y;$tcPropertyPatternName=$tcPropertyPatternName;tcPropertyPatternPattern=$tcPropertyPatternPattern_;testsuit=$SvrProjName;exec_realSemi_tafRedirectory=${localUrl}/$SvrProjName/$reportHtml_http\" title=\"$TCCtrToolTip\"> Exec</a>");
	...
	my $markTS =  sprintf("<a href=\"${localUrl}/$tafCGI?tafArgs=SysDrive=$SvrDrive;Execution_24_7=n;$tcPropertyPatternName=$tcPropertyPatternName;tcPropertyPatternPattern=$tcPropertyPatternPattern_;testsuit=$SvrProjName;mark_realSemi_tafRedirectory=${localUrl}/$SvrProjName/$reportHtml_http\" title=\"Make the beginning of a execution by \|\"> \|</a>");
	my $listTS =  sprintf("<a href=\"${localUrl}/$tafCGI?tafArgs=SysDrive=$SvrDrive;$tcPropertyPatternName=$tcPropertyPatternName;tcPropertyPatternPattern=$tcPropertyPatternPattern_;testsuit=$SvrProjName;testcaseNode=testcase;list_realSemi_tafRedirectory=${localUrl}/$SvrProjName/$reportHtml_http\" title=\"Update webUI /o Tags (default)\">S</a>");
	my $listTSAll=sprintf("<a href=\"${localUrl}/$tafCGI?tafArgs=SysDrive=$SvrDrive;$tcPropertyPatternName=$tcPropertyPatternName;tcPropertyPatternPattern=$tcPropertyPatternPattern_;testsuit=$SvrProjName;list_realSemi_tafRedirectory=${localUrl}/$SvrProjName/$reportHtml_http\" title=\"Update webUI /w Tags\">U</a>");
