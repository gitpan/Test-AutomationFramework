...
sub tcLoop {
	...
	while (($Execution_24_7 eq 'y') || ($NofExecution > $NofExecutionCtr)) {
		$NofExecutionCtr++;
		# my $cmd = sprintf "$c\\$_TAF\\taf.pl testsuit=$SvrProjName;mark"; print "pa:$cmd\n"; #my $rst =`$cmd`; 
...
sub recursiveSearchtcMain() { 
	...
	{	
		...
		if ($propertyOp !~ /^\s*$/) { printf "%20s\n", &processProperty($eachTC, $propertyOp); }  # PropertyManagement
			...
			( &getProperties(&getTCName($SvrTCName) , $tcPropertyPatternName) =~ /$tcPropertyPatternPattern/i))  { 		# Property/testcaseExec Filter
...
sub tsPostProcessPlugin {
	open Fin, $SvrDrive."/index.htm" || die "Can't open index.htm";
	while ($_ = <Fin>) { chop; if ( /\/\/\/(.+)index.htm/ ) { my $ts= $1; print "postProcessPlugin: $ts\n"; } }
}
...
sub createTS {
	...
	my $rst = mkpath $tsName;
	print " --> Create Testsuite: $tsName  ($rst) \n";
	$workingDir = $tsName;
...
EOF
	open Fout, "> $c/$_TAF/taf.bat";
	print Fout $str;
	close Fout;
	print " --> $c/$_TAF/taf.bat\n";
...
EOF
	...
	my $TCCtrToolTip = sprintf "Run Test Suite (Avg Time is $testsuiteTotalExecTime)"; 
	my $perl = $^X;  $perl =~ s/\\/\\\\/g;
	...
	my $tmp7 = sprintf("<a href=\"${urlHttp}/$SvrProjName/index_http.htm\" title=\"Run TAF-Team Version based on IIS/http\">");
...
${tmp9A}L</a>$passFailDisplay            <a title=\"Click Testcase Desc to view TC logs\">$strTmp</a>           ${tmp3}Pass</a>${tmp31}${tmp2}Fail</a>      ${tmp6} <a title=\"pass/fail counts\">      ${tmp1}Test Execution</a>     ${tmp7}Exec T</a>        <a title=\"Manual command to exec TC\">   ${tmp4}  </span>
EOF
...
# * TestSuite Machine independency, Move/Merge TAF over machine
...
# * 24 exec,  					done
# * Number of TS execution	setGlobal(NofExecution = 5)
# * start/stop gracefully	taf.pl exitTAF
# * 24 continuously, in stead of one test suite
# * Stop TAF graceffully from webUI 
# * bug: * history is incorrect 
